// pages/Productdetails/Productdetails.js - COMPLETE WITH CUSTOM FIELD + SHIPPING TIME
...
const Productdetails = () => { 
  ...
  useEffect(() => { 
    const fetchProduct = async () => { 
      ...
      try { 
        ...
        // ✅ Reset custom field input when product changes 
        setCustomFieldInput(""); 
        setCustomFieldError(""); 
  ...
  const handleAddToCart = async () => { 
    ...
    // ✅ Validate custom field if enabled 
    ...
    try { 
      ...
      await addToCart({ 
        ...
        // ✅ Custom field value typed by the customer 
  ...
  // ✅ Custom field — only needs enabled + label now 
  ...
  // Field appears only when enabled AND a label was set by the vendor 
        ...
        <Container className="py-5 lexend"> 
          ...
          <div className="product-accordion mt-5"> 
            <details open> 
              ...
              {/* 🆕 VENDOR DELIVERY TIME (uses shippingTime) */} 
              {/* {hasVendorDelivery && ( 
                <div 
                  className="vendor-delivery-time mt-3 p-3 border rounded" 
                  style={{ background: "#eef7ff", borderColor: "#b8daff" }} 
                > 
